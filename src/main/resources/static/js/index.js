    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/'+topic, function(data){
            ...
            if(jsonData['scene'] !== undefined){
                ...
                first = jsonData;
                if(jsonData['identified'] !== undefined){
                    renderFace('identified');
                }else if(jsonData['unrecognized'] !== undefined){
                    renderFace('unrecognized');
                }
            }
        });
    },function(message) {
...
function renderFace(data) {
    ...
    if (first) {
        var firstHtml = '<div class="portrait">' +
            '<img src="' + "data:image;base64,"+first[type] + '"/></div>' +
            '<div class="t_text">' +
    ...
    if (second) {
        var secondHtml = '<div class="portrait">' +
            '<img src="' + "data:image;base64,"+second[type] + '"/></div>' +
            '<div class="t_text">' +
    ...
    if (third) {
        var thirdHtml = '<div class="portrait">' +
            '<img src="' + "data:image;base64,"+third[type] + '"/></div>' +
            '<div class="t_text">' +
